...
results = {"targets": targets[loading_steps:loading_steps+test_steps], "predictions": predictions, "c": c,
           "config": {"N": N, "sr": sr, "bias": bias_scale, "in": in_scale, "p": density, "lam": lam, "lr": lr},
           "condition": {"k": k, "alpha": alpha, "repetition": rep},
           "training_error": epsilon}
pickle.dump(results, open(f"../results/clr_lorenz/k{k}_alpha{int(alpha)}_{rep}.pkl", "wb"))
