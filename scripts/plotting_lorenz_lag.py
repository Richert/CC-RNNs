...
for file in files:
    for model in models:
        ...
        df.loc[n, :] = (model, lag, rep, wd, k_star, dim, error)
        n += 1
...
for lag in lags:
    ...
    for model in models:
        ...
        # load data
        data = pickle.load(open(f"../results/{model}_lorenz/lag{lag}_{int(rep)}.pkl", "rb"))
...
# trajectory plots
titles = [f"lag = {n}" for n in lags] + ["Lorenz equations"]
colors = ["darkgreen", "darkblue", "darkred"]
model_titles = ["LR-RNN", "iLR-RNN"]
lvars = [0, 2]
...
# saving/plotting
fig.canvas.draw()
plt.savefig(f'../results/lorenz_lags.svg')
plt.show()
