...
class RNN(torch.nn.Module):
    ...
    def random_init(cls, N: int, n_in: int, sr: float = 1.0, density: float = 0.2, bias_var: float = 0.2,
                    rf_var: float = 1.0, tau_min: float = 0.9, tau_max: float = 1.0, dt: float = 0.01,
        ...
        tau = torch.tensor(tau_min + tau_max*np.random.rand(N), device=device, dtype=dtype)
...
class LowRankRNN(RNN):
    ...
    def __init__(self, W: torch.Tensor, W_in: torch.Tensor, bias: torch.Tensor, tau: torch.Tensor,
                 L: torch.Tensor, R: torch.Tensor, dt: float):

        super().__init__(W, W_in, bias, tau, dt)
        self.L = L
        self.R = R
        self.rank = L.shape[1]
